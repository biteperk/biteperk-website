...
- File: `../BitePerk_VoxTable_Deck_Print.pdf` — the 11 slides as full-page PDF.
...
- The editable original is `../BitePerk_VoxTable_Deck.pptx`.
...
**Rules of use:** only for venues actually running VoxTable — it's a badge, not an ad. Place it yourself during onboarding (door glass at eye level + one at the till). The QR is tagged `utm_source=venue-sticker` so you can count scans in your analytics.
