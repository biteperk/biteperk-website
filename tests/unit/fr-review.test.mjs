 ...
 * filed review byte for byte (the 15 Sep 2026 batch B today; batch A, 13 Sep and
 * 7 Sep are filed evidence that this config no longer regenerates — by design). That proves two things at once — the tool
...
const FILED = join(ROOT, "docs/ops-records/2026-09-15-french-review-ludovic-batch-b.md");
...
test("the extractor reproduces the filed 15 Sep batch-B review exactly", async () => {
...
test("every row carries both languages and they differ", async () => {
  ...
  assert.equal(total, 405, "the filed batch is 405 rows");
...
test("the page renders every extracted row, with the blocker marked", async () => {
  ...
  // Badge/blocking markup is driven by how many clusters are flagged blocking —
  // batch A had one (the contact form); batch B has none.
  const blockers = clusters.filter((c) => c.blocking?.startsWith("**Yes")).length;
  assert.equal((html.match(/class="badge"/g) ?? []).length, blockers, `${blockers} blocking badge(s)`);
  assert.equal((html.match(/class="cluster is-blocking"/g) ?? []).length, blockers);
});
...
test("--since walks all three copy modules, not just the ones that load easily", async () => {
  ...
  for (const file of ["copy.ts", "products.ts", "markets.ts", "solutions.ts", "resources.ts"]) {
