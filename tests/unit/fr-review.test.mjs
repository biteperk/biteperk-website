 ...
 * filed review byte for byte (the 15 Sep 2026 batch today; the 13 Sep and 7 Sep
 * ones are filed evidence that this config no longer regenerates — by design). That proves two things at once — the tool
...
const FILED = join(ROOT, "docs/ops-records/2026-09-15-french-review-ludovic.md");
...
test("the extractor reproduces the filed 15 Sep review exactly", async () => {
...
test("every row carries both languages and they differ", async () => {
  ...
  assert.equal(total, 31, "the filed batch is 31 rows");
