...
- **STATUS: the Vox rename is STAGED, not deployed.** It lives on branch `rename/vox-products` (committed) and must NOT ship until the VoxTable trademark clears. Live in production today is the **Perk** family (shipped 21 July 2026); deploying this branch replaces Perk with Vox. **Pre-deploy:** (1) clear the VoxTable trademark; (2) regenerate OG + composite images (`npm run og && npm run images`) so `vox*` assets exist — CI `check-images.mjs` fails otherwise; (3) merge to `main`, let CI build+deploy; (4) re-run Search Console for the `/products/vox*/` URLs.
