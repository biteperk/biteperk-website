...
- **STATUS: the Vox rename is STAGED, not deployed.** It lives on branch `rename/vox-products` (committed) and must NOT ship until the VoxTable trademark clears. Live in production today is the **Perk** family (shipped 21 July 2026); deploying this branch replaces Perk with Vox. **Pre-deploy:** (1) clear the VoxTable trademark — this is the only remaining blocker; (2) merge to `main` and watch CI green; (3) deploy functions first, then hosting (CI does **not** deploy — `web.yml` only builds and gates); (4) re-run Search Console for the `/products/vox*/` URLs, and re-scrape LinkedIn/Facebook share caches so old Perk cards don't linger. Assets are **done**: `og/vox*.png` and the `voxtable-hero-*` composites exist and are verified by `check-assets.mjs`.
