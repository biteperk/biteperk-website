...
> 🎨 **Producing anything that carries the BitePerk name — a page, document, artifact, deck, PDF, email?**
> **Read [`BRAND.md`](BRAND.md) first.** It is the portable brand kit: identity rules, the wordmark,
> the mark, the full token set for both themes, tone, and the NAP facts that must be byte-identical.
> Live source of truth for the site remains `src/styles/tokens.css`; `BRAND.md` is what you use when
> you can't import it.
...
- `node scripts/gates/check-schema.mjs` — asserts stable JSON-LD `@id`s (`#organization`, `#website`, `#vox`, per-city `#service`) and byte-exact Surry Hills NAP in the built HTML.
...
- `node scripts/gates/check-claims.mjs` — the AU-tree analog of `check-truthful` (which guards the *global* tree). Greps the **built** `dist/` — HTML *and* `.txt`, so `public/llms.txt` and other passthroughs are covered — for the one class of false data-claim the site keeps regressing to: that live call audio is *processed* onshore. It isn't (records are stored in Sydney; the voice-AI platform runs in the US), so `processed onshore`/`in Australia`, `call audio … stays in Australia`, `data stays in Australia`, `never leaves the country`, bare `onshore` not scoped to *records*, and the `train outside/external models` / `outside Biteperk` hedge all fail. Deliberately narrow — `records stored in Australia`, `records kept onshore`, `guest details staying in Australia` (stored records), `answered in … Australian region`, and `processed … in the United States` are all allowed. Runs on `dist/` so it can't be a src-only blind spot — the 30 Jul 2026 sweep that missed two card titles, an About card, two blogs and `llms.txt` is exactly the regression it catches. Fault-injected before trusting (`CLAIMS_DIST` points it at a fixture dir); AU leg only, since the global leg has no `dist/`.
- `node scripts/gates/check-analytics.mjs` — analytics gate, **both targets**. Asserts every built page carries exactly one self-hosted **Umami** tracker tag with the config's website id + `data-domains`, and `data-tag` = the base slug of the locale that owns the page (derived from `locales.ts`, so a locale added later is covered automatically); that no page references the removed `/api/event` endpoint; and (au leg) that the `biteperk-global` CSP lists `UMAMI_HOST` in **both** `script-src` and `connect-src` (a CSP miss silently blocks the tracker with no build error — nothing else catches it). Fault-injected (`ANALYTICS_DIST`/`FIREBASE_JSON`). Analytics is **cookieless, exempt, opt-out** — it does NOT arm `TRACKING_ARMED` (only marketing trackers do), and staging vs production is selected by the strict `UMAMI_ENV` build flag (set to `staging` only by `deploy-staging.yml`). Config SSOT: `src/data/consent.ts` (`UMAMI_*`).
- Then Playwright + axe (`npm run test:e2e`), Lighthouse budgets (`npm run lhci`), sitemap sanity, and lychee.

**Heading order is swept on BOTH targets** — `tests/a11y/axe.spec.ts` (AU) and `tests/intl/a11y.spec.ts` (global) each run a cheap no-axe `evaluate()` over every route asserting one `h1`, first, and no level jumps. This is separate from axe on purpose: axe rates `heading-order` *moderate* and both axe gates fail only on serious/critical, so **four** level skips lived on the site unnoticed — `h1→h3` on the AU home (TrustRow), `h1→h3` on every AU product page (feature cards), `h2→h4` in the AU footer, and `h1→h3` on all five intl contact pages. axe reported every one as advisory. Where a section's cards are genuinely subordinate but the section has no heading (product features), the fix is an `.sr-only` `h2` for the section, not promoting the cards.

**Accessibility on the intl trees** (`tests/intl/a11y.spec.ts`, project `intl-a11y`): axe over 12 routes × both themes — all five locale homes (chrome, consent, picker, FAQ) plus one page per distinct template from `/be-fr` plus `/gb-en/contact/` for the form. All 60 routes additionally get a no-axe heading sweep (one `h1`, first, no level jumps), because axe rates `heading-order` *moderate* and the AU suite gates only serious/critical — which is how an `h1 → h3` skip shipped on all five contact pages. The spec is `testIgnore`d from the three device projects so it runs once, not 3×. Both suites now gate heading order equally.
...
- **`src/data/intl/cities.ts` — the market city engine** (populated 29 Jul 2026: the eight UK cities — London, Manchester, Birmingham, Edinburgh, Glasgow, Leeds, Bristol, Liverpool — all `published` on `/gb-en`). Mirrors `cities.ts` but deliberately has **no geo/address/hours/phone field at all**: there is no European premises, and the absent field is the enforcement rather than a convention. Two image slugs per city (a unique cityscape + one hospitality shot shared across the market), not the AU engine's three — `public/images` is already 74MB and binaries never delta-compress. A `published` entry automatically gets its route, hreflang cluster (singleton, self-x-default — correct, `/en` doesn't emit cities), e2e coverage, sitemap entry at 0.9, footer + home-strip links, an llms-global line, a `check-schema` Service assertion, and an OG-card requirement (`buildGlobalCards()` derives `<slug>-gb-en.png`). The renderer is the `city` branch in `[...intl].astro`; its section furniture is `CopyBundle.cityPage` (language-scoped, `{city}` placeholder), the home strip copy is `MarketContent.cities` (market-scoped — a bundle field would break the parity unit test). **Copy landmine:** `check-truthful`'s NAP regex bans `Haymarket` and `Elizabeth Street` on every global page — also a London street, an Edinburgh district and a Belgravia dining street; they must never appear in city copy, districts or FAQs, and bare capitalised "Toast"/"Square" are best avoided entirely. Its city Service JSON-LD carries **no** `offers`/`areaServed`/`geo`/`address` — the AU city node has all four and copying it fails two gates. French city-page labels in `copy.ts` had Ludovic's pass on **7 Sep 2026** (verbal) — the gate on `/fr` and `/be-fr` city publishing is **discharged**. No French city is written yet; Paris and Brussels-FR are the next candidates.
- **Copy = language cores × market overrides** (`src/data/intl/`): `copy.ts` holds the full EN/FR bundles and must stay **market-neutral** (France-specific lines live in the `/fr` override or `be-fr` inherits them); `markets.ts` layers per-base overrides + the imagery band (cityscape/hospitality slugs + alts); `index.ts` resolves via `merge()` — **arrays replace wholesale** (contract unit-tested in `tests/unit/`, run by `gates:global` and CI). French (core + `/fr` + `/be-fr` + the intl product copy) had Ludovic's native review and is **signed off (26 Jul 2026)** — this no longer gates `INTL_LAUNCHED`. French added *after* that date needs its own pass. The 27 Jul 2026 market-differentiation French (both FR home pages, both about pages, two five-item FAQs) had that pass and is **signed off (27 Jul 2026)** — cleared for prospect-facing use. Everything French written between then and 7 Sep — chrome labels, consent, the call transcript, the trust strip, the city-page furniture, VoxStay, the AI Act citation clauses — was reviewed as one 117-line batch and **passed with no corrections on 7 Sep 2026**. That pass was **verbal**, so unlike the July ones no email backs it; the batch is filed at `docs/ops-records/2026-09-07-french-review-ludovic.md`. The rule stands for French written after it. **The review document that pass is recorded against is regenerable**: `node scripts/build/extract-fr-review.mjs --out <file>` rebuilds it from the live modules (clusters are declared at the top of that script — editing `CLUSTERS` is the whole interface for a new batch), and `node scripts/build/build-review-page.mjs --out <file>.html` renders the shareable page from the *same* extraction. `tests/unit/fr-review.test.mjs` asserts the extractor still reproduces the filed 7 Sep review byte for byte — when that test fails because copy changed, the fix is a NEW dated review under `docs/ops-records/`, never editing the filed one. **To find what a new batch must cover, don't trust memory: `--since <git-ref>` diffs every French leaf string against that ref and lists what is new or changed** — the AI Act citation clauses were written and shipped the same morning as the 7 Sep batch and appeared in no cluster until someone remembered them; `--since 20c3042` reports exactly those two.
...
- **Europe-truthful rules (PLAN.md §8): no AU price, no NAP, no AU phone on any global page** — enforced twice: `check-schema` (global) asserts the shared AU-anchored org `@id`, Organization-only, no address/geo/areaServed on EVERY locale home; `check-truthful` sweeps every built global page's HTML for the phone numbers, both AU NAPs (current Surry Hills and pre-move Haymarket — stale copy resurfacing is as much a leak as new) and `$80` (this sweep caught the AU phone hiding in the global org JSON-LD — structured data is part of the page). CTA is "book a pilot". No fake local offices — market pages localise the conversation and imagery, never invent a presence.
- **A UK company now exists, and it changes less than it looks like it should (rev. 7 Aug 2026).** **Biteperk Ltd** — company **17379647**, registered in England and Wales, registered office **124 City Road, London EC1V 2NX**, incorporated 4 Aug 2026 — is the contracting party and data controller for UK customers. It is a **registered office, not a premises**: there is still no European staff, no European phone line and no VAT registration, so *every* rule above stands unchanged. Two `check-truthful` entries are load-bearing and must never be relaxed to accommodate it: the `+44/+33/+32` ban (there is no UK line, and E-Commerce Regs reg 6 wants an *email*, not a number) and the `our London office/team/staff` ban (a registered office is neither). What the UK entity gets is scoped to `/gb-en` and nothing else — footer disclosure, `legal/company-details`, the `#uk` JSON-LD node, and market-scoped privacy/terms. `check-schema` asserts that node is **present on `/gb-en` and absent on the other four trees**: a London address on `/fr` would be true and still wrong. Source of record is **Companies House**, never a third-party aggregator — those publish *inferred* size/turnover bands for companies that have filed no accounts (Biteperk Ltd's first are due 4 May 2028), and republishing an inference as fact is what `check-claims` exists to stop.
- `node scripts/gates/check-uk-disclosure.mjs` — the UK statutory-disclosure gate (`gates:global`). **Positive**, unlike most gates here, so it carries a page-count floor: a positive gate that finds no files passes vacuously. Asserts every `dist-global/gb-en/**` page carries the registered name, place of registration, company number and registered office (SI 2015/17 reg 25 attaches to "websites", and the footer is the only surface on every page), that `legal/company-details` exists and carries an **email address** plus the Companies House link (E-Commerce Regs 2002 reg 6 — a contact form does **not** satisfy reg 6(1)(c)), and that **no VAT number is claimed**. Fault-injected before trusting (`UK_DIST` points it at a fixture dir).
...
- `src/data/site.ts` — name, URL, phone, email, **NAP address** (Level 1/457-459 Elizabeth St, Surry Hills NSW 2010 — moved from Haymarket 28 Jul 2026), hours, social. `abn`/`acn` are real (registered with ASIC 29 Jul 2026) — the `"TODO"` note here was stale. `social` is an ordered `SocialProfile[]`: **every** profile goes into the org `sameAs`, only those flagged `visible` get a footer icon. `entities` (sibling export) holds the AU and UK legal-entity facts; `ukEmail` is the `/gb-en` contact address.
...
- `inlineStylesheets: "always"` (every page's CSS is inlined — there is no render-blocking stylesheet request, at the cost of HTML bytes, which is why the `AuBase` import-graph hygiene matters) and `prefetch.defaultStrategy: "viewport"` are tuned for hero LCP — don't undo without reason. This line said `"auto"` for months; the config never did.
...
**Picking a stock photo:** judge it from the GRADED file in `public/images/`, never a side-channel fetch — a candidate preview once didn't match the Unsplash ID it claimed, and only looking at the graded output caught it. Weigh encode cost during selection (`sharp(src).resize(1280).avif({quality:58,effort:6})`): dark frames full of small bright highlights run 3-4× heavier, which matters in the hero slot because Lighthouse's mobile config picks the 1280 variant and the intl LCP element is *text*. `fetch-images.mjs` takes an optional `rect` (`"x,y,w,h"` in the ORIGINAL's pixels — get them with `?w=8000&fit=max`) to pin a crop in the catalogue so a clean checkout reproduces it. Finally, **look at the picture next to the words**: the old `/gb-en` hero showed chairs stacked on tables beside a headline promising the phone is answered *during service*, and the old UK band showed a US bar with dollar prices on the chalkboard.
...
- `docs/marketing-engine.md` — weekly/monthly/quarterly SEO + off-site cadence; also the open launch blockers (ABN, GBP verification; analytics resolved 2026-09-14 — self-hosted Umami replaced Plausible).
...
| Products | **Vox** (umbrella), **VoxTable**, **VoxOrder**, **VoxConcierge**, **VoxStay** (hotels, in development), **VoxDrive** (concept) | Yes — this is the family that renamed. CamelCase, "by BitePerk". **VoxStay** went from pitch-only to a public `in-development` product on 5 Sep 2026 — see the VoxStay note below and `BRAND.md`. |
| Persona | **Bella** | Never. |
...
| Audience | Domain | Email | Signature block |
|---|---|---|---|
| **Australia** (AU venues, AU directories, AU press, GBP, print) | `biteperk.com.au` | `sales@biteperk.com.au`, `hello@biteperk.com.au` | `biteperk.com.au` + `+61 2 5504 1140` |
| **Rest of world** (the five international locales — `/en`, `/gb-en`, `/fr`, `/be-en`, `/be-fr` — and any non-AU prospect, partner or hotel pitch) | `biteperk.com` | `sales@biteperk.com` | `biteperk.com`, **no AU phone number** |

Rules that follow from this:

- **Never mix them in one message.** A `@biteperk.com.au` sender with a `biteperk.com` signature (or the reverse) reads as a spoof and hurts deliverability. Sender address, signature domain and any link in the body must all be on the same side of the table.
- **AU phone stays on the AU side.** `+61 2 5504 1140` and the AU NAP must not appear in email to international prospects — same Europe-truthful rule the `check-truthful` gate enforces on the global build (PLAN.md §8). There is no European premises and no European line; the CTA there is "book a pilot".
- **CRM follow-up:** Zoho lead records carry the audience. AU-sourced leads (Online Store, AU directories, GBP) get replied to from the `.com.au` identity; international leads from `.com`. The Zoho "from" address is per-send — check it before hitting Send, it does not follow the lead.
- **Deliverability caveat (open):** `biteperk.com.au` is **not yet SPF/DKIM-authenticated in Zoho**, so Zoho rewrites the sender through its own domain on every send and shows a "Sender email changed" banner. Authenticate both domains in Zoho Mail admin — this is a live deliverability tax on every AU outbound email. Owner: Sam.
- This rule is about **correspondence identity only**. It does not change `src/data/site.ts` (`site.email` stays `hello@biteperk.com.au`, the AU site's published address), the `.com.au` → `.com` 301 redirect host, or the four-entry `ALLOWED_ORIGINS` list in `functions/index.js` — those are site architecture and are documented above.

- **VoxStay is on the site (rev. 5 Sep 2026)** — fifth catalogue entry, `in-development`, all six trees, `/products/voxstay/`. The product is real (`~/voxstay`, a working demo, EU-first on Google Cloud Paris) but has **no live hotel, no metrics and no pricing**, so every public claim is architectural: eight languages, real price with taxes, texted secure payment link, never a card number on the call, cannot invent a price, falls back to ringing the front desk, *designed* for EU hosting. Never on the site: prospect names (WiZiU / Accor / Covivio / Ludovic), any hotel PMS/OTA name (`check-truthful` has a **case-sensitive** entry for them — `mews` is a London street word and `Opera` is a Sydney building, which is why it is not folded into the `/i` restaurant-integration rule), any price or currency, "our Paris team" in either language, the +61 test number. It is the exception to the AU tree's "Australian data residency" line and the copy says so where that line appears (`products/index.astro` featureList, `llms.txt`). French for VoxStay (`src/data/intl/products.ts`, `markets.ts` about-intro tweak) had Ludovic's **verbal pass on 7 Sep 2026** — no longer draft, cleared for prospect-facing use. `tests/unit/intl-products.test.mjs` now guards the product copy (slug parity, no English in FR, h1 ≤ 45 chars for the OG card, per-product photo) — `intl-merge.test.mjs` never covered this module. Layout for five: the mega-menu and the `/products/` capability grid span the first (core) card and lay the four add-ons 2×2; the home ProductGrid is 2×2.
...
- **`marketing/` collateral (rev. 7 Sep 2026).** The **A5 flyer and A5 brochure are v2**: `npm run collateral` renders HTML templates through Playwright Chromium into `marketing/flyer_and_broucher/v2/pdf/` — brand fonts (static Inter + Source Serif 4, embedded as CID TrueType), `tokens.css` hex, the exported mark, real photography, and every NAP/phone/URL imported from `src/data/site.ts` via `_load-ts.mjs` so print cannot drift from the site. It replaced reportlab (not installable on the Mac's Python 3.14, and unable to load the brand woff2s). `check.mjs` gates each build: banned strings (`vocotable`, Haymarket, prices, `+44`, Natalia, the onshore-processing claims), 8.5 pt floor, safe area, 4.5:1 contrast, 250 ppi, blank-render, **QR decoded from the rendered preview and the URL must answer**, and a **DRAFT lock** — the Mazcina Resto-Bar quote is Camilo & Mauro's to approve, and until `testimonial.approved` is true the `-PRINT-Officeworks.pdf` files are not written. The v1 flyer (`flyer_and_broucher/v1/`, `build_flyer.py`) printed **`vocotable.biteperk.com.au`, which no longer resolves**, and the banned AI portrait `marketing/bella.png` — do not print it. **Still reportlab/hand-built and not yet rebuilt:** `print/build_pack_extras.py` (cards, stickers, follow-up, and the demo card, which still uses `bella.png`), the 11-slide deck (PPTX + PDF, no generator), the walk-in demo kit and the pitch script. Those are the next phase; when they move, they move onto the v2 pipeline, not back to reportlab. Colour on the v1 brochure/pack is `#f8b406`/`#0f4f35` sampled from a JPEG — not the brand.
