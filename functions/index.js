...
// pushed into Zoho CRM: test enquiries are not pipeline. `biteperk-global.web.app`
// (the raw production origin) stays deliberately absent; preview channels are
// matched by PREVIEW_ORIGIN below.
const STAGING_ORIGINS = ["https://biteperk-staging.web.app"];
const ALLOWED_ORIGINS = [...PRODUCTION_ORIGINS, ...STAGING_ORIGINS];
// Firebase Hosting preview channels (`firebase hosting:channel:deploy <name>`)
// serve from https://biteperk-global--<name>-<hash>.web.app. They are the
// review surface for every PR now, so the form must work there; the raw
// biteperk-global.web.app origin stays out on purpose.
const PREVIEW_ORIGIN = /^https:\/\/biteperk-global--[a-z0-9-]+\.web\.app$/;
const originAllowed = (origin) => ALLOWED_ORIGINS.includes(origin) || PREVIEW_ORIGIN.test(origin);
// NB: this is a same-site check, not CORS. No Access-Control-* headers are
// ever sent (cors: false below), so a genuinely cross-origin browser POST
// cannot read the response regardless; the allowlist rejects a foreign
// Origin outright so a hostile page cannot even fire-and-forget leads.
...
const { LOCALE_BASES, COUNTRY_BY_LOCALE, audienceOf, mailboxFor, thankYouHtml, errorHtml } = require("./pages");
...
exports.contactForm = onRequest(
  ...
  async (req, res) => {
    ...
    // Which tree served the form. Allowlisted against locales.ts bases (via
    // pages.js); anything else stores as null rather than as free text.
    const localeRaw = str(b.locale, 20);
    const locale = LOCALE_BASES.includes(localeRaw) ? localeRaw : null;
    const audience = audienceOf({ crmForm, locale });
    const ctx = { audience, locale };
    if (
      !name ||
      !EMAIL_RE.test(email) ||
      !message ||
      ...
      return finish(res, false, 400, error, wantsJson, ctx);
    }

    // Per-connection rate limit (see RATE_LIMIT), counted AFTER validation so
    // five typo'd attempts from one venue's NAT do not lock the sixth, valid
    // one out. The IP is used transiently here and reaches Firestore only as
    // a hash on the counter document.
    ...
    if (await rateLimited(clientIp)) {
      return finish(
        ...
        false,
        429,
        `Too many messages from your connection — please wait a few minutes, or email ${mailboxFor(audience)}.`,
        wantsJson,
        ctx,
      );
    }
    ...
    try {
      ...
      return finish(res, false, 500, `Sorry — something went wrong. Please email ${mailboxFor(audience)}.`, wantsJson, ctx);
    }
    ...
    if (zohoGlobal && quality === "ok" && environment === "production") {
      try {
        await submitZohoLead({ name, email, venue, product, message, locale });
      } catch (e) {
    ...
    return finish(res, true, 200, null, wantsJson, ctx);
  }
);
...
async function submitZohoLead({ name, email, venue, product, message, locale }) {
  const body = new URLSearchParams({
    ...
    // Market routing: Zoho's standard Country field where the tree implies
    // one, and the tree itself on the first line of the description so an
    // /en (x-default) lead is still attributable.
    ...(COUNTRY_BY_LOCALE[locale] ? { Country: COUNTRY_BY_LOCALE[locale] } : {}),
    Description: locale ? `Market: ${locale}\n\n${message}` : message,
...
function finish(res, ok, status, error, wantsJson, ctx = { audience: "au", locale: null }) {
  if (wantsJson) {
    return res.status(ok ? 200 : status).json(ok ? { ok: true } : { ok: false, error });
  }
  res.set("Content-Type", "text/html; charset=utf-8");
  return res.status(ok ? 200 : status).send(ok ? thankYouHtml(ctx) : errorHtml({ error, ...ctx }));
}

// escapeHtml / thankYouHtml / errorHtml live in ./pages.js (pure, unit-tested).
