...
>
> **Scope note — what this pass did NOT cover (now resolved):** French flagged "next Ludovic batch" that was never actually in a cluster of this document — `TrustFactsCopy` fr `authorityLabel`/`authorityBody`/`teamLabel` (`copy.ts`), and the whole of `intl/solutions.ts` and `intl/resources.ts` — was assembled as **batch B** and passed the same day. See `docs/ops-records/2026-09-15-french-review-ludovic-batch-b.md`.
