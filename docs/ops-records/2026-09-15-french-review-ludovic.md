> **PASSED — Ludovic verbal pass, 15 September 2026, no corrections.** Sam notified Ludovic and he reviewed and passed everything in this document; recorded here and in `docs/accounts-and-ops-log.md`. This re-sent the 13 Sep batch (which had not yet been passed) with one string added: `chrome.languageRegion` ("Langue et région"), the MobileDrawer "Language & region" row label from the unified mobile chrome work. The full set is every French string written after Ludovic's 7 Sep verbal pass (found with `extract-fr-review.mjs --since origin/main`, not from memory). The 31 rows below are what was passed — the `DRAFT`/"NOT yet reviewed" flags on these strings in `copy.ts` and `markets.ts` are now dropped. **Do not edit the rows below**; they are what was reviewed. As before, this pass was **verbal**, so no email backs it. The 13 Sep file (`2026-09-13-french-review-ludovic.md`) stays as filed evidence of the earlier send.
>
> **Scope note — what this pass does NOT cover:** French flagged "next Ludovic batch" that was never actually in a cluster of this document is still unreviewed: `TrustFactsCopy` fr `authorityLabel`/`authorityBody`/`teamLabel` (`copy.ts`), and the whole of `intl/solutions.ts` and `intl/resources.ts`. Those need their own batch (add them to `CLUSTERS` in `extract-fr-review.mjs`, regenerate, and send) before they are prospect-facing.
...
| `languageRegion` | Language & region | Langue et région | |
