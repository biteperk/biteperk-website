> **Sent for review, 15 September 2026 — NOT yet passed.** This re-sends the 13 Sep batch (which had not yet been passed) with one string added: `chrome.languageRegion` ("Langue et région"), the MobileDrawer "Language & region" row label from the unified mobile chrome work. The full set is every French string written after Ludovic's 7 Sep verbal pass (found with `extract-fr-review.mjs --since origin/main`, not from memory). Until it is passed, the source marks each string as unreviewed and the branches carrying it (`feat/unified-mobile-chrome`, and the still-pending #69, fix/intl-structure, fix/fr-copy) must not go to prod via an `INTL_LAUNCHED` deploy. When the pass lands, record it here and in `docs/accounts-and-ops-log.md` — do not edit the rows below; they are what was sent. The 13 Sep file (`2026-09-13-french-review-ludovic.md`) stays as filed evidence of the earlier send.
